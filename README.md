...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 改造项目** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。

## 功能模块

| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、最新问题与巡查、进行中改造项目数 |
...
| 改造项目 | `/renovations`、`/renovations/:id` | 立项登记（改造事由/立项时间/施工单位/计划工期/预算）、节点进度与阶段验收结论、开工自动停用公厕、完工验收恢复开放、项目档案永久保留、工期超期预警 |
...
| GET | `/renovations` | 改造项目查询（restroom_id/district/status/关键字/排序/分页） |
| POST | `/renovations` | 立项登记，自动生成编号 `GZ-YYYYMMDD-001` 并写入首条档案流水 |
| GET/PATCH | `/renovations/{id}` | 项目详情（含完整项目档案）/ 更新（完工或取消后不可改） |
| GET | `/renovations/{id}/transitions` | 当前状态可执行的项目动作 |
| POST | `/renovations/{id}/transitions` | 推进项目状态（开工/申请完工验收/完工验收/取消） |
| POST | `/renovations/{id}/milestones` | 记录节点进度与阶段验收结论（仅改造中） |
...
- **改造项目**：`已立项 → 改造中 → 待完工验收 → 已完工`；`待完工验收` 可退回 `改造中` 继续整改，`已立项/改造中` 可取消。开工时公厕状态自动转为「暂停使用」（并记录改造前状态），完工验收通过或项目取消后自动恢复。同一公厕同一时间只允许一个进行中的项目，重复立项返回 409。
- **项目档案**：节点进度（节点名称、整体进度百分比、阶段验收结论）与流程事件（立项、开工、验收、取消）统一写入项目档案流水，完工后项目转为只读归档，不提供删除接口，档案永久可查。
- **工期超期**：计划完工时间已过且项目仍未完工归档时，项目列表与详情页显示「工期超期」。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、2 个改造项目（老城隍庙公厕综合改造已完工归档、第三小学旁公厕无障碍改造进行中并联动停用）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
