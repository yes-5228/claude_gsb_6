...
面向城市公厕管养单位的巡查记录、整改闭环与改造跟踪管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 改造项目** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。

## 功能模块

| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、最新问题与巡查、改造项目指标 |
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 改造项目）、关联数据删除保护 |
...
| 改造项目 | `/projects`、`/projects/:id` | 立项登记（事由/施工单位/计划工期/预算）、立项自动停用公厕、进度节点与阶段验收结论、竣工验收后恢复公厕状态、项目档案只读封存 |
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / issues / projects / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、改造项目/节点
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、改造项目、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / issues / projects 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| GET | `/projects` | 改造项目查询（restroom_id/district/status/open_only/delayed/立项日期区间/关键字） |
| POST | `/projects` | 立项登记，编号 `GZ-YYYYMMDD-001`，公厕自动转为暂停使用 |
| GET/PATCH/DELETE | `/projects/{id}` | 详情（含进度节点与流转轨迹）/ 更新登记信息 / 删除（仅进行中，恢复公厕状态） |
| GET | `/projects/{id}/transitions` | 当前状态可执行的流转动作 |
| POST | `/projects/{id}/transitions` | 推进项目状态（越级流转返回 400） |
| POST | `/projects/{id}/nodes` | 登记进度节点（仅施工中） |
| PATCH | `/projects/{id}/nodes/{node_id}` | 登记/复验节点阶段验收结论（合格/不合格） |
| DELETE | `/projects/{id}/nodes/{node_id}` | 删除进度节点（仅施工中） |
| GET | `/stats/overview` | 核心指标（含改造项目累计/进行中/已验收） |
| GET | `/stats/dashboard` | 看板聚合数据（趋势、分布、区域、排行、最新记录） |
| GET | `/meta/dictionaries` | 枚举字典（状态、分类、程度、检查项、整改与改造流转规则等） |
...
- **改造停用与恢复**：立项登记成功时自动把公厕置为「暂停使用」并保存立项前状态；项目流程为 `待施工 → 施工中 → 待验收 → 已验收`，待验收可被驳回退回施工中。竣工验收通过后公厕**恢复立项前状态**（可能是正常开放或维修中等），记录验收意见与结算造价。
- **进度节点与阶段验收**：仅「施工中」可增删进度节点（节点名称、日期、完成度、进度说明）；每个节点可单独登记阶段验收结论（合格/不合格 + 验收意见 + 验收人），施工中与待验收阶段均可复验，复验覆盖原结论。
- **工期超期**：项目尚未竣工验收且计划完工日期早于当天时，列表与详情页显示「工期超期」，可按 `delayed=true` 过滤。
- **档案封存**：竣工验收后的项目档案只读，编辑、删除、追加节点、继续流转均返回 400；仅进行中的项目可删除，删除时同步恢复公厕立项前状态。同一公厕存在进行中项目时禁止重复立项。
- **改造联动护栏**：公厕暂停使用期间不能登记巡查；公厕存在改造项目时删除同样受到 409 删除保护，需 `force=true` 级联删除。
- **删除保护**：删除公厕时若已存在巡查、问题记录或改造项目，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、4 个不同阶段的公厕改造项目（已验收封存 / 施工中工期超期 / 待施工 / 待验收，含进度节点与阶段验收结论）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
